...
void Renderer::CreateComputeDescriptorSetLayout() {
  ...
  VkDescriptorSetLayoutBinding blades_layout_binding_out = {};
  blades_layout_binding_out.binding = 1;
  blades_layout_binding_out.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
  blades_layout_binding_out.descriptorCount = 1;
  blades_layout_binding_out.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
  blades_layout_binding_out.pImmutableSamplers = nullptr;

  VkDescriptorSetLayoutBinding blades_layout_binding_count = {};
  blades_layout_binding_count.binding = 2;
  blades_layout_binding_count.descriptorType =
      VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
  blades_layout_binding_count.descriptorCount = 1;
  blades_layout_binding_count.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
  blades_layout_binding_count.pImmutableSamplers = nullptr;

  std::vector<VkDescriptorSetLayoutBinding> bindings = {
      blades_layout_binding, blades_layout_binding_out,
      blades_layout_binding_count};
  ...
  if (vkCreateDescriptorSetLayout(logicalDevice, &create_info, nullptr,
...
void Renderer::CreateDescriptorPool() {
  ...
  std::vector<VkDescriptorPoolSize> poolSizes = {
      ...
      // TODO: Add any additional types and counts of descriptors you will need
      // to allocate

      // Compute
      {VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
       3 * static_cast<uint32_t>(scene->GetBlades().size())},  // grass blades
...
void Renderer::CreateGrassDescriptorSets() {
  ...
  if (vkAllocateDescriptorSets(logicalDevice, &alloc_info,
  ...
  for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
    ...
    model_buffer_info.buffer = scene->GetBlades()[i]->GetModelBuffer();
    model_buffer_info.offset = 0;
    model_buffer_info.range = sizeof(ModelBufferObject);

    descriptor_writes[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptor_writes[i].dstSet = grassDescriptorSets[i];
    descriptor_writes[i].dstBinding = 0;
    descriptor_writes[i].dstArrayElement = 0;
    descriptor_writes[i].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    descriptor_writes[i].descriptorCount = 1;
    descriptor_writes[i].pBufferInfo = &model_buffer_info;
...
void Renderer::CreateComputeDescriptorSets() {
  ...
  compute_descriptor_sets_.resize(scene->GetBlades().size());

  VkDescriptorSetLayout layouts[] = {compute_descriptor_set_layout_};
  VkDescriptorSetAllocateInfo allocInfo = {};
  allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
  allocInfo.descriptorPool = descriptorPool;
  allocInfo.descriptorSetCount =
  ...
  if (vkAllocateDescriptorSets(logicalDevice, &allocInfo,
                               compute_descriptor_sets_.data()) != VK_SUCCESS) {
    throw std::runtime_error("Failed to allocate descriptor set");
  }

  std::vector<VkWriteDescriptorSet> descriptor_writes(
      3 * compute_descriptor_sets_.size());

  for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
    VkDescriptorBufferInfo compute_buffer_info = {};
    compute_buffer_info.buffer = scene->GetBlades()[i]->GetBladesBuffer();
    compute_buffer_info.offset = 0;
    compute_buffer_info.range = sizeof(Blade) * NUM_BLADES;

    VkDescriptorBufferInfo compute_buffer_info_out = {};
    compute_buffer_info_out.buffer =
        scene->GetBlades()[i]->GetCulledBladesBuffer();
    compute_buffer_info_out.offset = 0;
    compute_buffer_info_out.range = sizeof(Blade) * NUM_BLADES;

    VkDescriptorBufferInfo compute_buffer_info_count = {};
    compute_buffer_info_count.buffer =
        scene->GetBlades()[i]->GetNumBladesBuffer();
    compute_buffer_info_count.offset = 0;
    compute_buffer_info_count.range = sizeof(BladeDrawIndirect);

    descriptor_writes[3 * i + 0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptor_writes[3 * i + 0].dstSet = compute_descriptor_sets_[0];
    descriptor_writes[3 * i + 0].dstBinding = 0;
    descriptor_writes[3 * i + 0].dstArrayElement = 0;
    descriptor_writes[3 * i + 0].descriptorType =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    descriptor_writes[3 * i + 0].descriptorCount = 1;
    descriptor_writes[3 * i + 0].pBufferInfo = &compute_buffer_info;
    descriptor_writes[3 * i + 0].pImageInfo = nullptr;
    descriptor_writes[3 * i + 0].pTexelBufferView = nullptr;

    descriptor_writes[3 * i + 1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptor_writes[3 * i + 1].dstSet = compute_descriptor_sets_[0];
    descriptor_writes[3 * i + 1].dstBinding = 1;
    descriptor_writes[3 * i + 1].dstArrayElement = 0;
    descriptor_writes[3 * i + 1].descriptorType =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    descriptor_writes[3 * i + 1].descriptorCount = 1;
    descriptor_writes[3 * i + 1].pBufferInfo = &compute_buffer_info_out;
    descriptor_writes[3 * i + 1].pImageInfo = nullptr;
    descriptor_writes[3 * i + 1].pTexelBufferView = nullptr;

    descriptor_writes[3 * i + 2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    descriptor_writes[3 * i + 2].dstSet = compute_descriptor_sets_[0];
    descriptor_writes[3 * i + 2].dstBinding = 2;
    descriptor_writes[3 * i + 2].dstArrayElement = 0;
    descriptor_writes[3 * i + 2].descriptorType =
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    descriptor_writes[3 * i + 2].descriptorCount = 1;
    descriptor_writes[3 * i + 2].pBufferInfo = &compute_buffer_info_count;
    descriptor_writes[3 * i + 2].pImageInfo = nullptr;
    descriptor_writes[3 * i + 2].pTexelBufferView = nullptr;
  }
...
void Renderer::CreateGraphicsPipeline() {
  VkShaderModule vertShaderModule =
      ShaderModule::Create("shaders/graphics.vert.spv", logicalDevice);
  VkShaderModule fragShaderModule =
      ShaderModule::Create("shaders/graphics.frag.spv", logicalDevice);
...
void Renderer::CreateGrassPipeline() {
  ...
  VkShaderModule vertShaderModule =
      ShaderModule::Create("shaders/grass.vert.spv", logicalDevice);
  VkShaderModule tescShaderModule =
      ShaderModule::Create("shaders/grass.tesc.spv", logicalDevice);
  VkShaderModule teseShaderModule =
      ShaderModule::Create("shaders/grass.tese.spv", logicalDevice);
  VkShaderModule fragShaderModule =
      ShaderModule::Create("shaders/grass.frag.spv", logicalDevice);
...
void Renderer::CreateComputePipeline() {
  ...
  VkShaderModule computeShaderModule =
      ShaderModule::Create("shaders/compute.comp.spv", logicalDevice);
...
void Renderer::RecordComputeCommandBuffer() {
  ...
  vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                          ...
                          &compute_descriptor_sets_[0], 0, nullptr);

  vkCmdDispatch(computeCommandBuffer,
                ceil((NUM_BLADES + WORKGROUP_SIZE - 1) / WORKGROUP_SIZE), 1, 1);
  // ~ End recording ~
...
Renderer::~Renderer() {
  ...
  vkDestroyDescriptorSetLayout(logicalDevice, compute_descriptor_set_layout_,
                               nullptr);
