...
Renderer::Renderer(Device* device, SwapChain* swapChain, Scene* scene, Camera* camera)
	...
	CreateGrassDescriptorSetLayout(); // Grass Layout
...
void Renderer::CreateGrassDescriptorSets() {
	int nBlades = scene->GetBlades().size();
	grassDescriptorSets.resize(nBlades);
	...
	for (uint32_t i = 0; i < nBlades; ++i) {
		
		VkDescriptorBufferInfo modelBufferInfo = {};
		VkDescriptorImageInfo zInfo = {};

		modelBufferInfo.buffer = scene->GetBlades()[i]->GetModelBuffer();
		modelBufferInfo.offset = 0;
		modelBufferInfo.range = sizeof(ModelBufferObject);
		//zInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		//zInfo.imageView = depthImageView;
...
void Renderer::CreateComputeDescriptorSets() {
	int nBlades = scene->GetBlades().size();
	computeDescriptorSets.resize(nBlades);
