...
void Renderer::CreateGrassDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        if (vkCreateSampler(device->GetVkDevice(), &samplerInfo, nullptr, &imageInfo.sampler) != VK_SUCCESS) {
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        culledBladesInfo.range = NUM_BLADES * sizeof(Blade);
        ...
        descriptorWrites[3 * i + 1].pBufferInfo = &culledBladesInfo;
