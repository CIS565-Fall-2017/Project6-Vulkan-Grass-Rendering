...
void Renderer::CreateGrassDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { uboLayoutBinding, depthLayoutBinding };
...
void Renderer::CreateGrassDescriptorSets() {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		//Image::TransitionLayout(device, graphicsCommandPool, depthImage, depthFormat, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, depthInfo.imageLayout);
		//depthTextureView = Image::CreateView(device, depthImage, depthFormat, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		...
		if (vkCreateSampler(device->GetVkDevice(), &samplerInfo, nullptr, &depthInfo.sampler) != VK_SUCCESS) {
		...
		descriptorWrites[2 * i + 1].dstBinding = 1;
		descriptorWrites[2 * i + 1].dstArrayElement = 0;
		descriptorWrites[2 * i + 1].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
		descriptorWrites[2 * i + 1].descriptorCount = 1;
		descriptorWrites[2 * i + 1].pImageInfo = &depthInfo;
		*/
	}
...
void Renderer::CreateComputeDescriptorSets() {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		VkDescriptorBufferInfo culledBaldesInfo = {};
		culledBaldesInfo.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
		culledBaldesInfo.offset = 0;
		culledBaldesInfo.range = NUM_BLADES * sizeof(Blade);

		VkDescriptorBufferInfo numBaldesInfo = {};
		numBaldesInfo.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
		numBaldesInfo.offset = 0;
		numBaldesInfo.range = sizeof(BladeDrawIndirect);
		...
		descriptorWrites[3 * i + 0].pBufferInfo = &bladesInfo;
		...
		descriptorWrites[3 * i + 1].pBufferInfo = &culledBaldesInfo;
		...
		descriptorWrites[3 * i + 2].pBufferInfo = &numBaldesInfo;
...
void Renderer::CreateFrameResources() {
	...
	depthFormat = device->GetInstance()->GetSupportedFormat({ VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT }, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
	// CREATE DEPTH IMAGE
	Image::Create(device,
